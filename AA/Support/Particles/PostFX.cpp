...
void postfx::AddFX   ( const vector3& Pt, const vector2& Size, const xcolor& Color, f32 Life )
{
    ASSERT( Pt.InRange( -20000, 20000 ) );
...
void postfx::AddFX   ( const vector3& Pt1, const vector3& Pt2, f32 Size, const xcolor& Color )
{
    ASSERT( Pt1.InRange( -20000, 20000 ) );
    ASSERT( Pt2.InRange( -20000, 20000 ) );
...
void postfx::Render  ( void )
{
    ...
    s32 i ;
    f32 DeltaTime = tgl.DeltaLogicTime; //(tgl.LogicTimeMs % 1000) / 1000.0f ;
    ...
    if ( s_Glow == -1 )
        s_Glow = tgl.GameShapeManager.GetTextureManager().GetTextureIndexByName("[A]GlowFX") ;

    // make it the current texture    
    draw_SetTexture( tgl.GameShapeManager.GetTextureManager().GetTexture(s_Glow).GetXBitmap() );
    ...
    for ( i = 0; i < MAX_POST_FX; i++ )
    {
        ...
        //if ( m_FX[ i ].m_Type == postfx_element::SPOT )
        {
            ...
            if ( m_FX[ i ].m_Life > 0.0f )
            {
                ...
                if ( CheckPointVis( m_FX[ i ].m_Pt[0] ) )
