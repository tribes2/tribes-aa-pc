...
void view::SetYFOV( radian YFOV )
{
    ...
    m_ScreenDist = HalfVPHeight / x_tan( YFOV * 0.5f );
    ...
    m_XFOV = x_atan( HalfVPWidth / m_ScreenDist ) * 2.0f;
    m_YFOV = YFOV;
...
void view::GetMinMaxZ( const bbox& BBox, f32& MinZ, f32& MaxZ ) const
{
    ...
    MinZ = m_ZPlane.Normal.X * pF[m_ZPlaneMinI[0]] +
           m_ZPlane.Normal.Y * pF[m_ZPlaneMinI[1]] +
           m_ZPlane.Normal.Z * pF[m_ZPlaneMinI[2]] +
           m_ZPlane.D;

    MaxZ = m_ZPlane.Normal.X * pF[m_ZPlaneMaxI[0]] +
           m_ZPlane.Normal.Y * pF[m_ZPlaneMaxI[1]] +
           m_ZPlane.Normal.Z * pF[m_ZPlaneMaxI[2]] +
           m_ZPlane.D;
}
                          ...
                          system System ) const
{
    ...
    vector3 PTL  = vector3(  L,  T,  m_ScreenDist );
    vector3 PTR  = vector3(  R,  T,  m_ScreenDist );
    vector3 PBL  = vector3(  L,  B,  m_ScreenDist );
    vector3 PBR  = vector3(  R,  B,  m_ScreenDist );
                                    ...
                                    f32      Radius ) const
{
    ...
    f32     DeltaLen2 = Delta.X*Delta.X + Delta.Y*Delta.Y + Delta.Z*Delta.Z;
                                  ...
                                  system   System /* = WORLD */ ) const
{
    ...
    return ((f32)m_ShotSize * WorldRadius * 2 * m_ScreenDist)/ZDist;
}
...
f32 view::GetScreenDist( void ) const
{
    ...
    return m_ScreenDist * m_ShotSize ;  // Take shot into account for correct PS2 MipK
}
...
void view::UpdateScreenDist( void ) const
{
    if( m_Dirty & DIRTY_SCREENDIST )
    {
        ...
        // Compute dist using XFOV.
        f32 HalfVPWidth  = (m_ViewportX1 - m_ViewportX0 + 1) * 0.5f;
        m_ScreenDist = HalfVPWidth / x_tan( m_XFOV * 0.5f );
    }
}
