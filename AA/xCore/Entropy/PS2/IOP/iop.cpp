...
#define NETCNF_ICON_FILE "icon=cdrom:\\TRIBES\\MODULES\\CDNETCNF\\SYS_NET.ICO;1" "\0" "iconsys=cdrom:\\TRIBES\\MODULES\\CDNETCNF\\ICON.SYS;1"
...
#define INET_CONFIG_PARAMS "mem=64KB"
...
s32 iop_LoadModule (char *module_name,char *pArg=NULL,s32 arglength=0,xbool AllowFail=FALSE)
{
	...
	x_sprintf(Buff,"host:%s/entropy/ps2/modules-PAL/%s",XCORE_PATH,module_name);
#else
	x_sprintf(Buff,"host:%s/entropy/ps2/modules/%s",XCORE_PATH,module_name);
#endif
#elif defined(DEMO_DISK_HARNESS)
	x_sprintf(Buff,"cdrom0:\\TRIBES\\MODULES\\%s;1",module_name);
	x_strtoupper(&Buff[6]);
#else
...
void iop_Init(s32 IopBufferSize,s32 MaxRequestsPending /*= MAX_PENDING_IOP_REQUESTS*/)
{
	...
	if (s_PCMCIA_Enabled)
	{
		g_pIopVars->m_pPoweroffThread = new xthread(PoweroffThread,"Power off Handler",8192,8);
	}
#endif
