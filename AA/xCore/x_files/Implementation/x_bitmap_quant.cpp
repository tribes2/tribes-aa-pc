...
void quant_End( xcolor* pPalette, s32 aNColors, xbool UseAlpha )
{
    ...
    else
    {
        ...
        AI = (alpha_info*)x_malloc(sizeof(alpha_info)*256);
        AB = (alpha_box*)x_malloc(sizeof(alpha_box)*256);
        AP = (alpha_pal*)x_malloc(sizeof(alpha_pal)*256);
        ASSERT(AI && AB && AP);
        x_memset(AI,0,sizeof(alpha_info)*256);
        x_memset(AB,0,sizeof(alpha_box)*256);
        x_memset(AP,0,sizeof(alpha_pal)*256);
        NColors = s_NPixels;
        pColor = s_Pixel;
        while( NColors-- )
        {
