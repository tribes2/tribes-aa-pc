...
void* x_malloc( s32 NBytes )
{
    ...
    if( pHeader == NULL )
    {
        ...
        else
        {
            ...
            scePrintf("Allocation failed: size=%d, free=%d, largest=%d,fragments=%d\n",NBytes,Free,Largest,Fragments);
            return NULL;
        }
    }
#endif
...
void* x_realloc( void* pMemory, s32 NewNBytes )
{
    ...
    if( pHeader == NULL )
    {
        ...
        else
        {
            s32 Free,Largest,Fragments;
            ps2_GetFreeMem(&Free,&Largest,&Fragments);
            x_DebugMsg("Allocation failed: size=%d, free=%d, largest=%d,fragments=%d\n",NewNBytes,Free,Largest,Fragments);
            return NULL;
        }
    }
#endif
...
void ps2_GetFreeMem(s32 *pFree,s32 *pLargest,s32 *pFragments)
{
	...
	scePrintf("ps2_GetFreeMem: ---------------------Begins-------------------------\n");
#endif
...
#ifdef SHOW_MEM_BLOCKS
		scePrintf("%08x: Length: %08x: Flag %c\n",(u32)pHeapBase, length, "UA"[flag]);
#endif
	...
	*pFree = Free;
	s_MemFree = Free;
	s_LargestFree = LargestFree;
	s_Fragments = Fragments;

#ifdef SHOW_MEM_BLOCKS
	scePrintf("ps2_GetFreeMem: ---- End: %d bytes free, %d in use, %d fragments, %d largest, total %d\n",Free,Used,Fragments,LargestFree,Free+Used);
#endif
...
void ps2_InitHeap(void)
{
	...
	free(temp);
	scePrintf("ps2_InitHeap: Heap size 0x%08x bytes (%08x expected), base at 0x%08x, top at 0x%08x\n",size,maxsize,(s32)s_BeginHeapMark,(s32)s_EndHeapMark);
