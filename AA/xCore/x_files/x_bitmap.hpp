...
public:
    ...
    struct format_info
    {
        ...
        char    String[16];     // String version of Format
        xbool   ClutBased;      // Is a clut used?
        s32     BPP;            // Bits Per Pixel
        s32     BPC;            // Bits Per Color
        s32     BitsUsed;       // Bits used
        s32     RBits;          // Bits of Red
        s32     GBits;          // Bits of Green
        s32     BBits;          // Bits of Blue
        s32     ABits;          // Bits of Alpha
        s32     UBits;          // Bits of Unused
        s32     RShiftR;        // Shift right for Red
        s32     GShiftR;        // Shift right for Green
        s32     BShiftR;        // Shift right for Blue
        s32     AShiftR;        // Shift right for Alpha
        s32     UShiftR;        // Shift right for Unused
        s32     RShiftL;        // Shift left  for Red
        s32     GShiftL;        // Shift left  for Green
        s32     BShiftL;        // Shift left  for Blue
        s32     AShiftL;        // Shift left  for Alpha
        s32     UShiftL;        // Shift left  for Unused
...
const   xbitmap&    operator =      ( const xbitmap& Bitmap );
        ...
        void        BuildMips           ( s32    MipsDesired = 15  );
