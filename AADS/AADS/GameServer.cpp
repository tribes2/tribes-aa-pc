...
void game_server::HandleLoginPacket( bitstream& BitStream, net_address& SenderAddr )
{
    ...
    if( i==MAX_CLIENTS )
    {
        ...
        if( GameMgr.RoomForPlayers( Login.NPlayers ) )
        {
            ...
            m_nClients++;
			ASSERT(Login.NPlayers <= 2);

            for( j=0; j<Login.NPlayers; j++ )
            {
                // Store name
				ASSERT(x_wstrlen(Login.Data[j].Name) <=32);
...
//                  GameMgr.EnterGame( PlayerIndex );
					...
					g_TelnetManager.ClearStatistics(PlayerIndex);
                }
            }
...
void game_server::ProcessPacket( bitstream& BitStream, net_address& SenderAddr )
{
    ...
    case CONN_PACKET_TYPE_DISCONNECT:
        {
            s32     i;
            s32     KickVote = GameMgr.GetKickTarget();
            xbool   Kick     = FALSE;    
            ...
            // See if a kick is in progress on one of the players disconnecting.
            for( i=0; i<MAX_CLIENTS; i++ )
            {
                if( (m_Client[i].Connected) && (m_Client[i].Address == SenderAddr) )
                {
                    if( (m_Client[i].PlayerIndex[0] == KickVote) )
                    {
                        m_Client[i].Kick = TRUE;
                        Kick = TRUE;
                    }

                    if( (m_Client[i].NPlayers == 2) &&
                        (m_Client[i].PlayerIndex[0] == KickVote) )
                    {
                        m_Client[i].Kick = TRUE;
                        Kick = TRUE;
                    }
...
void game_server::CullClient( s32 ID )
{
    ...
    if( m_Client[ID].Kick )
    {
        BanClient( m_Client[ID].SystemId );
    }
...
void game_server::ProcessTime( f32 DeltaSec )
{
	...
	if ( (GetAveragePing() > 1000.0f) && (g_SM.CurrentState == STATE_SERVER_INGAME) )
	{
			...
			if (s_KickTimer.ReadSec() > s_KickTimeout)
			{
				for( i=0; i<MAX_CLIENTS; i++ )
				{
					if( m_Client[i].Connected && (m_Client[i].ConnManager.GetDestState() == STATE_CLIENT_INGAME) )
					{
						...
						g_ClientsDropped++;
						char ipstr[64];
						net_IPToStr(m_Client[i].Address.IP,ipstr);
						g_TelnetManager.AddDebugText("game_server: Dropped connection %s because too busy\n",ipstr);
						break;
					}
				}
				s_KickTimer.Stop();
...
#if 0
            //****** THIS IS HOW TO SEND A FREE FORMAT TEXT MESSAGE TO THE CLIENTS
            if( m_Client[i].ConnManager.GetDestState() == STATE_CLIENT_INGAME )
            {
                    if ( s_MessageTimer.IsRunning() && (s_MessageTimer.ReadSec() > 4.0f) )
                    {
    // Build a message packet
                            {
                                ...
                                BS.WriteS32( 0, 4 );
                                BS.WriteColor( (xcolor)m_Client[i].PlayerIndex[0] );
                                BS.WriteWString(xwstring("Welcome to the Inevitable server"));
