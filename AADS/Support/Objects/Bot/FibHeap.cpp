...
s32  FibHeapNode::FHN_Cmp(FibHeapNode& RHS)
{
    if (NegInfinityFlag)
...
s32  FibHeapNode::operator ==(FibHeapNode& RHS)
{
...
void FibHeapNode::Draw( s32 VertexSpacing) const
{
    eng_Begin();
    draw_Sphere(
        vector3( (f32)cr.X * VertexSpacing
            , 2.0f
            , (f32)cr.Z * VertexSpacing )
        , 10.0f, XCOLOR_WHITE );
    eng_End();
}
...
s32  FibHeap::DecreaseKey(FibHeapNode *theNode, FibHeapNode& NewKey)
{
...
s32  FibHeap::Delete(FibHeapNode *theNode)
{
    FibHeapNode Temp;
    s32 Result;
...
void FibHeap::Print(FibHeapNode *Tree, FibHeapNode *theParent)
{
    ...
    do {
        if (Temp->Left == NULL)
            x_printf( "(Left is NULL)" );
        Temp->Print();
        if (Temp->Parent != theParent)
            x_printf( "(Parent is incorrect)" );
        if (Temp->Right == NULL)
            x_printf( "(Right is NULL)" );
        else if (Temp->Right->Left != Temp)
            x_printf( "(Error in left link left) ->" );
        else x_printf( " <-> " );
    ...
    if (theParent == NULL)
    {
        ...
        x_printf( "Done Printing.  Hit a key.\n" );
        cin >> ch;
    }
}
#endif // TESTING_FIB_HEAP
...
void FibHeap::Draw( FibHeapNode *Tree ) const
{
    x_printf( "Drawing %d nodes\n", NumNodes );
    FibHeapNode* Temp = NULL;
...
{
    FibHeapNode *x, *y, *w;
    FibHeapNode *A[1+8*sizeof(s32)]; // 1+lg(n)
    s32 I=0, Dn = 1+8*sizeof(s32);
    s16 d;
    ...
    do {
        //x_printf( "Top of Consolidate's loop\n" );
        //Print(w);
        ...
        while (A[d] != NULL)
        {
            ...
            A[d] = NULL;
            d++;
            //x_printf( "After a round of Linking\n" );
            //Print(x);
        }
        A[d] = x;
